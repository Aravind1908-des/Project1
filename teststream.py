    ...
    if Role_1 == "Support":
        ...
        if closure_query:
            ...
            if st.button("**Update Status**"):
               cursor.execute("UPDATE clientdetails SET status = %s WHERE query_id_number = %s",
               (closure_status, closure_query))
               conn.commit()
        ...
        if st.button("**New data**"):
            ...
            column_names = ['name', 'query_id_number', 'Email', 'Number', 'heading', 'description']
